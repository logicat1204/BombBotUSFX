...
// Sets default values
AWorld3_Factory::AWorld3_Factory()
{
...
void AWorld3_Factory::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AWorld3_Factory::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AWorld3_Factory::CreateMap()
{
	//Creacion de la parte superior del mapa
	AlturaBloques = 1600.0f;
	CreateBlocks(BasicMap, AlturaBloques);
}

void AWorld3_Factory::CreateTerrain()
{
...
void AWorld3_Factory::CreateEnemy(int32 CantidadEnemigos)
{
...
void AWorld3_Factory::CreatePowerUps(int32 CantidadPowerUps)
{
	// Buscar todas las posiciones libres (valor 0)
	TArray<FIntPoint> PosicionesLibres;
	for (int32 i = 0; i < MapaCompleto.Num(); i++)
...
void AWorld3_Factory::CreatePortals()
{
}

void AWorld3_Factory::StartPosition()
{
...
TArray<TArray<int32>> AWorld3_Factory::FullMap(TArray<TArray<int32>> MapaInicial) {
...
void AWorld3_Factory::CreateBlocks(TArray<TArray<int32>> FullMap, int32 Altura)
{
	...
	for (int32 i = 0; i < Filas; i++)
	{
		for (int32 j = 0; j < Columnas; j++)
		{
			...
			case 2:
				if (FabricaBloques)
				{
					FabricaBloques->CrearBloque("BloqueMadera3", Posicion);
				}
				break;
			...
			case 4:
				if (FabricaBloques)
				{
					FabricaBloques->CrearBloque("BloquePiedra3", Posicion);
				}
				break;
			case 5:
